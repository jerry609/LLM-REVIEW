...
**✅ 今日检查**：能默写 Attention 的 shape 流转：`Q[B,H,S,D] × K^T[B,H,D,S] → [B,H,S,S] × V[B,H,S,D] → [B,H,S,D]`
...
| 60min | 精读 RoPE 数学推导 | 📖 | `math_dictionary/rope-and-position-encoding.md` |
| 30min | 精读 RoPE 扩展：PI、NTK-Aware、YaRN | 📖 | `notes/attention/long-context.md` |
| 30min | **口述练习**：「RoPE 如何实现相对位置编码？为什么不需要额外参数？」 | 🎤 | — |

**✅ 今日检查**：能手画 Llama3 单层 decoder block 的完整流程图
...
### Day 6 — Tokenizer + Sampling + LoRA
| 时间 | 任务 | 类型 | 对应文件 |
|------|------|------|---------|
| 40min | 精读 Tokenizer 数学 | 📖 | `math_dictionary/tokenizer-math.md` |
| 40min | 精读概率与采样 | 📖 | `math_dictionary/probability-and-sampling.md` |
| 50min | 精读 LoRA/PEFT 数学 + 笔记 | 📖 | `math_dictionary/lora-peft-math.md` + `notes/training/lora-rlhf.md` |
| 30min | 运行 LoRA + Continuous Batching 部分 | 🧑‍💻 | `notebooks/llm_inference_fundamentals.ipynb`（后半部分） |
| 20min | 运行 LoRA 源码 | 🧑‍💻 | `src/training/lora.py` |

**✅ 今日检查**：能解释 LoRA 的参数量计算 `Δ = rank × (d_in + d_out)`
...
### Day 8 — Phase 1 复盘 + Scaling Law + 阶段自测
| 时间 | 任务 | 类型 | 对应文件 |
|------|------|------|---------|
| 40min | 精读 Scaling Law + 优化 | 📖 | `math_dictionary/optimization-and-scaling.md` + `notes/training/scaling-law.md` |
| 40min | 精读对齐流程 | 📖 | `notes/training/alignment-pipeline.md` |
| 30min | 复习 D1-D7 全部口述笔记，查漏补缺 | 📖 | 回顾笔记 |
...
### Day 35 — 前沿：DeepSeek + Qwen + Kimi
| 时间 | 任务 | 类型 | 对应文件 |
|------|------|------|---------|
| 50min | 精读 DeepSeek-V3/R1 笔记 | 📖 | `notes/frontier/deepseek-v3-r1.md` |
| 40min | 精读 Qwen 系列笔记 | 📖 | `notes/frontier/qwen-series.md` |
| 40min | 精读 Kimi/Moonshot 笔记 | 📖 | `notes/frontier/kimi-moonshot.md` |
| 30min | 精读 GLM/智谱笔记 | 📖 | `notes/frontier/glm-zhipu.md` |
| 20min | 精读 MiniMax-01 笔记 | 📖 | `notes/frontier/minimax-01.md` |
| 20min | **口述练习**：「对比 DeepSeek-V3 vs Qwen2.5 vs Kimi 的技术路线差异」 | 🎤 | — |

**✅ 今日检查**：能列出每家公司的核心技术差异点（各 3 个关键词）

---

### Day 36 — 前沿面试模拟 + 结构化输出 + 多模态
| 时间 | 任务 | 类型 | 对应文件 |
|------|------|------|---------|
| 30min | 精读结构化输出笔记 | 📖 | `notes/frontier/structured-output.md` |
| 30min | 精读 VLM Serving 架构 | 📖 | `notes/multimodal/vlm-serving.md` |
| 45min | **面试模拟**：Reasoning Models 题 | 🧪 | `mock_interview/by-topic/frontier-q001-reasoning-models.md` |
| 30min | **面试模拟**：模型对比题 | 🧪 | `mock_interview/by-topic/frontier-q002-model-comparison.md` |
| 30min | **面试模拟**：长上下文题 | 🧪 | `mock_interview/by-topic/frontier-q003-long-context.md` |
| 15min | 精读 Production Runbook | 📖 | `notes/debugging/production-runbook.md` |

**✅ 今日检查**：前沿面试题能答到 7/10 分以上
...
| D34-36 | `notes/frontier/` (全部 9 篇), `notes/multimodal/`, `notes/debugging/` |
| D38 | `notes/coding/` |
